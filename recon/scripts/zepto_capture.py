"""
mitmproxy addon to capture and log Zepto API traffic.

Usage:
    mitmproxy -s zepto_capture.py
    mitmdump -s zepto_capture.py -w ../captured/zepto_flows.flow
"""
...
class ZeptoCapture:
    ...
    def _is_zepto(self, host: str) -> bool:
        return any(domain in host for domain in ZEPTO_DOMAINS)

    def _is_interesting(self, path: str) -> bool:
        return any(keyword in path.lower() for keyword in INTERESTING_PATHS)

    def response(self, flow: http.HTTPFlow):
        if not self._is_zepto(flow.request.host):
            return
